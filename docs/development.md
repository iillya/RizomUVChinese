...
只接入宿主程序目录内已加载模块的公开 GDI 导入槽，按已解析的函数地址识别，支持缺少名称表、DLL 别名和延迟导入。每两秒检查一次后加载模块和已解析的延迟导入，不覆盖第三方已替换的槽；首次调用可能仍显示英文。

词典在首次接入前加载，此后不可变。绘制回调复用线程局部查找缓冲，直接引用词典译文，不读磁盘、不写日志。不要增加每帧全窗口遍历或联网翻译。
