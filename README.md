...
4. 点击“安装汉化”。
5. 安装完成后，从桌面或开始菜单打开“RizomUV 简体中文版”。
...
请始终使用“RizomUV 简体中文版”快捷方式启动。直接运行原版 `rizomuv.exe` 不会加载汉化，这是正常现象。
...
安装新版前先关闭 RizomUV，然后直接运行新版安装器并再次点击“安装汉化”即可。安装器会安全替换旧版汉化文件；如果安装中途失败，会尽量保留或恢复上一版。

## 拆卸与恢复原版

1. 保存工程并关闭 RizomUV。
2. 再次运行同一个安装器。
3. 选择当初安装汉化的 RizomUV 目录。
4. 点击“拆卸汉化”。

拆卸只会删除本补丁创建的 `ChineseLauncher` 文件夹和快捷方式，不会卸载 RizomUV，也不会删除用户工程。

如果安装器检测到 RizomUV 仍在后台运行，会询问是否结束进程。强制结束可能丢失未保存内容，请先确认工程已经保存。

## 常见问题

### 安装后仍然是英文

- 确认启动的是“RizomUV 简体中文版”，不是原版快捷方式。
