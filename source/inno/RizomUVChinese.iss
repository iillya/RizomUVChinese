...
FinishedLabel=中文补丁已安装。请通过“RizomUV 中文版”快捷方式启动软件。%n%n卸载请使用 Windows“已安装的应用”。卸载会清除补丁文件及备份，保留您另行创建或修改的词典和文件。
...
[Icons]
#ifndef TestMode
Name: "{autodesktop}\RizomUV 中文版"; Filename: "{app}\ChineseLauncher\RizomUVChineseLauncher.exe"; WorkingDir: "{app}"; Tasks: desktopicon
Name: "{autoprograms}\RizomUV 中文版"; Filename: "{app}\ChineseLauncher\RizomUVChineseLauncher.exe"; WorkingDir: "{app}"; Tasks: startmenuicon
#endif
...
begin
  Result := '"' + InstallRoot + '\RizomUVChineseLauncher.exe" "%1"';
end;
...
begin
  ...
  if LegacySid = '' then begin
    SetLength(Owner, 185);
    if not LegacyOwner(InstallRoot + '\RizomUVChineseLauncher.exe', Owner, 185) then begin
...
begin
  if CurUninstallStep = usUninstall then begin
    try
      ...
      if not RestoreLegacy(Sid, InstallRoot + '\RizomUVChineseLauncher.exe') then
